...
class Products extends CI_Controller {
    ...
    public function getProducts(){
        ...
        if(isset($data->idFrom) && $data->idFrom != '' && isset($data->idTo) && $data->idTo != ''){
    ...
    public function getProductStock(){
        ...
        if(isset($data->productId) && $data->productId != null){
            ...
            $stock = $this->db->query("
                ...
                left join tbl_unit u on u.Unit_SlNo = p.Unit_ID
                where p.status = 'a' and p.is_service = 'false' and Product_branchid = '$branchId'
                $clauses
                order by p.Product_Name asc
            ")->result();
        }
    ...
    public function getAvailableExpDate() {
        $inputs = json_decode($this->input->raw_input_stream);
        $branchId = $this->session->userdata("BRANCHid");

        $productId = $inputs->productId;
        $clauses = "";
        if(isset($inputs->expired_available) && $inputs->expired_available != 0 && $inputs->expired_available != "" && $inputs->expired_available != null) {
            $clauses .= " and pd.expire_date >= '" . date('Y-m-d') . "' ";
        }
        $expireDate = 0;

        $purchaseQty = $this->db->query("
            select
             p.Product_Name, 
             p.Product_SlNo as product_number, 
             pd.PurchaseDetails_TotalQuantity as purchase_quantity,
             pd.expire_date,
             (
                select ifnull(sum(pd1.PurchaseDetails_TotalQuantity), 0) 
                from tbl_purchasedetails pd1 where pd1.Product_IDNo = p.Product_SlNo 
                and pd1.Status = 'a' 
                and pd1.expire_date = pd.expire_date 
            ) as purchased_quantity,
            (
                select ifnull(sum(sd.SaleDetails_TotalQuantity), 0) 
                from tbl_saledetails sd where sd.Product_IDNo = p.Product_SlNo
                and sd.Status = 'a' 
                and sd.expire_date = pd.expire_date 
            ) as sold_quantity,
            ( 
                select ifnull(sum(prd.PurchaseReturnDetails_ReturnQuantity), 0) 
                from tbl_purchasereturndetails prd where prd.PurchaseReturnDetailsProduct_SlNo = p.Product_SlNo 
                and prd.Status = 'a' 
                and prd.PurchaseReturnDetails_ProductExp_date = pd.expire_date 
            ) as purchase_returned_quantity,
            (
                select ifnull(sum(srd.SaleReturnDetails_ReturnQuantity), 0) 
                from tbl_salereturndetails srd where srd.SaleReturnDetailsProduct_SlNo = p.Product_SlNo 
                and srd.Status = 'a' 
                and srd.SaleReturnDetails_ProductExp_date = pd.expire_date 
            ) as sales_returned_quantity,
            ( 
                select ifnull(sum(dmg.DamageDetails_DamageQuantity), 0) 
                from tbl_damagedetails dmg where dmg.Product_SlNo = p.Product_SlNo 
                and dmg.status = 'a' and dmg.damage_productExpire = pd.expire_date 
            ) as damaged_quantity,
            (select (purchased_quantity + sales_returned_quantity) - (sold_quantity + purchase_returned_quantity + damaged_quantity)) as current_quantity,
            (select p.Product_Purchase_Rate * current_quantity) as stock_value

             from tbl_purchasedetails pd join tbl_product p on p.Product_SlNo = pd.Product_IDNo 
             where p.Product_SlNo = ? and pd.PurchaseDetails_branchID = ?
             $clauses
             group by pd.expire_date"
        , [$productId, $this->session->userdata("BRANCHid")])->result();

        echo json_encode($purchaseQty);
    }
    ...
    public function getCurrentStock(){
        ...
        $stock = array_map(function($product) {
            $product->quantity_text = $product->per_unit == 0 ? $product->current_quantity .' '. $product->Unit_Name : floor($product->current_quantity / $product->per_unit) . ' ' . $product->convert_text . ' '. $product->current_quantity % $product->per_unit . ' ' . $product->Unit_Name;
            return $product;
        }, $stock);
    ...
    //     $expList = $this->db->query("
    //         select p.Product_Code, p.Product_Name, p.expire_date, pc.ProductCategory_Name 
    //         from tbl_product p, tbl_productcategory pc
    //         where p.ProductCategory_ID = pc.ProductCategory_SlNo
    //         and p.status = 'a'
    //         and p.Product_branchid = ?
    //         and p.expire_date BETWEEN '$date' and '$nextDate'
    //         order by p.expire_date asc
    //     ", $this->session->userdata('BRANCHid'))->result();

    //     echo json_encode($expList);
    // }

    public function getPurchaseExpireList() {
        $date = date('Y-m-d');
        $nextDate = date('Y-m-d', strtotime("+30 days"));
        
        $expList = $this->db->query("
            select pm.PurchaseMaster_InvoiceNo, p.Product_Code, p.Product_Name, pc.ProductCategory_Name,
             pd.AddTime, pd.expire_date, 
            (select DATEDIFF(pd.expire_date, '$date') + 1) as remaining_date
                from tbl_purchasedetails pd
                join tbl_purchasemaster pm on pm.PurchaseMaster_SlNo = pd.PurchaseMaster_IDNo
                join tbl_product p on p.Product_SlNo = pd.Product_IDNo
                join tbl_productcategory pc on pc.ProductCategory_SlNo = p.ProductCategory_ID
                and p.status = 'a' and pd.Status = 'a'
                and pd.PurchaseDetails_branchID = ?
                and pd.expire_date BETWEEN '$date' and '$nextDate'
                group by pd.expire_date, pd.Product_IDNo
                order by p.expire_date asc
        ", $this->session->userdata('BRANCHid'))->result();
    ...
    public function getTopSold(){
        ...
        $clauses = "";
        if(isset($data->soldType) && $data->soldType != ''){
            $clauses .= "$data->soldType";
        }

        $products = $this->db->query("
            select p.Product_Code, p.Product_Name, pc.ProductCategory_Name, sum(sd.SaleDetails_TotalQuantity) as soldQty
            from tbl_saledetails sd, tbl_product p, tbl_productcategory pc
            where sd.Product_IDNo = p.Product_SlNo
            and p.ProductCategory_ID = pc.ProductCategory_SlNo
            and sd.SaleDetails_BranchId = ?
            group by sd.Product_IDNo
            order by sum(sd.SaleDetails_TotalQuantity) $clauses limit 100
        ", $this->session->userdata('BRANCHid'))->result();
    ...
    public function getExpireStock() {
        ...
        $stock = $this->db->query("
            select 
                p.*, pc.ProductCategory_Name, pm.PurchaseMaster_SlNo,
                u.Unit_Name, pd.PurchaseDetails_SlNo, 
                pd.PurchaseDetails_TotalQuantity, pd.expire_date as Purchase_ExpireDate, 
                (
                    select ifnull(sum(pd1.PurchaseDetails_TotalQuantity), 0) 
                    from tbl_purchasedetails pd1 where pd1.Product_IDNo = p.Product_SlNo 
                    and pd1.Status = 'a' 
                ...
                left join tbl_productcategory pc on pc.ProductCategory_SlNo = p.ProductCategory_ID
                left join tbl_unit u on u.Unit_SlNo = p.Unit_ID
                where pd.Status = 'a' and p.is_service = 'false' and p.Product_branchid = ? $clauses  group by pd.expire_date, pd.Product_IDNo order by pd.PurchaseDetails_SlNo DESC
   ...
   function barcode($kode){
    ...
    public function getProductLedger(){
        ...
        $result = $this->db->query("
            ...
            " . (isset($data->expireAvailable) && $data->expireAvailable != null && $data->expireAvailable == 1  ? " and pd.expire_date >= '$date'" : "") . "
            ...
            " . (isset($data->expireAvailable) && $data->expireAvailable != null && $data->expireAvailable == 1 ? " and sd.expire_date >= '$date'" : "") . "
            ...
            " . (isset($data->expireAvailable) && $data->expireAvailable != null && $data->expireAvailable == 1  ? " and prd.PurchaseReturnDetails_ProductExp_date >= '$date'" : "") . "
            ...
            " . (isset($data->expireAvailable) && $data->expireAvailable != null && $data->expireAvailable == 1 ? " and srd.SaleReturnDetails_ProductExp_date >= '$date'" : "") . "
            ...
            " . (isset($data->expireAvailable) && $data->expireAvailable != null && $data->expireAvailable == 1 ? " and dmd.damage_productExpire >= '$date'" : "") . "
