...
class Purchase extends CI_Controller
{
    ...
    public function getPurchases(){
        ...
        if(isset($data->purchaseId) && $data->purchaseId != null){
            ...
            $purchaseDetails = array_map(function($detail) {
                $detail->quantity_text = floor($detail->PurchaseDetails_TotalQuantity / $detail->per_unit) .' '. $detail->convert_text. ' ' . $detail->PurchaseDetails_TotalQuantity % $detail->per_unit . ' '. $detail->Unit_Name;
                return $detail;
            }, $purchaseDetails);
        }
    ...
    function addDamage()
    {
        ...
        try {
            $data = json_decode($this->input->raw_input_stream);
            if(isset($data->expire_date) && $data->expire_date != null && $data->expire_date != "") {
            ...
            $damageDetails = array(
                ...
                'damage_productExpire' => $expireDate,
                'status' => 'a',
                'AddBy' => $this->session->userdata("FullName"),
                'AddTime' => date('Y-m-d H:i:s')
            );
    ...
    public function getDamages(){
        ...
        $damages = $this->db->query("
            ...
            where d.status = 'a' and dd.status = 'a' and d.Damage_brunchid = ?
            $clauses
        ", $this->session->userdata('BRANCHid'))->result();
