    ...
    public function getSales()
    {
        ...
        if (isset($data->salesId) && $data->salesId != 0 && $data->salesId != '') {
            ...
            $saleDetails = array_map(function ($detail) {
                $detail->quantity_text = floor($detail->SaleDetails_TotalQuantity / $detail->per_unit) . ' ' . $detail->convert_text . ' ' . $detail->SaleDetails_TotalQuantity % $detail->per_unit . ' ' . $detail->Unit_Name;
                return $detail;
            }, $saleDetails);
        }
        $sales = $this->db->query("
        ...
        try {
            ...
            foreach ($data->cart as $product) {
                $returnDetails = array(
                    ...
                    'SaleReturnDetails_ProductExp_date' => $product->expire_date,
                    'Status' => 'a',
                    'AddBy' => $this->session->userdata("FullName"),
                    'AddTime' => date('Y-m-d H:i:s'),
