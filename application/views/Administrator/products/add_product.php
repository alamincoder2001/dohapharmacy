...
<div id="products">
		...
		<div class="row" style="margin-top: 10px;margin-bottom:15px;border-bottom: 1px solid #ccc;padding-bottom: 15px;">
			<div class="col-md-6">
				...
				<div class="form-group clearfix">
					<label class="control-label col-md-4">Genetic:</label>
					<div class="col-xs-11 col-md-7">
						<v-select v-bind:options="genetices" v-model="selectedGenetic" label="name"></v-select>
					</div>
					<div class="col-xs-1 col-md-1" style="padding:0;margin-left: -15px;"><a href="/genetic" target="_blank" class="add-button"><i class="fa fa-plus"></i></a></div>
				</div>
				<div class="form-group clearfix">
					<label class="control-label col-md-4">Company:</label>
					<div class="col-xs-11 col-md-7">
						<v-select v-bind:options="companies" v-model="selectedCompany" label="name"></v-select>
					</div>
					<div class="col-xs-1 col-md-1" style="padding:0;margin-left: -15px;"><a href="/company" target="_blank" class="add-button"><i class="fa fa-plus"></i></a></div>
				</div>
				<div class="form-group clearfix">
				...
				<div class="form-group clearfix">
					<label class="control-label col-md-4">Shelf Location:</label>
					<div class="col-md-7">
						<input type="text" class="form-control" v-model="product.shelf">
					</div>
				</div>
			</div>	
			...
			<div class="col-md-6">
				...
				<div class="form-group clearfix">
					<label class="control-label col-md-4">Convertion Name:</label>
					<div class="col-md-7">
						<input type="text" class="form-control" v-model="product.convert_text" required>
					</div>
				</div>

				<div class="form-group clearfix">
					<label class="control-label col-md-4">Per Unit:</label>
					<div class="col-md-7">
						<input type="number" class="form-control" min="1" v-model="product.per_unit" required>
					</div>
				</div>
	...
	new Vue({
		...
		watch: {
			...
			selectedGenetic(genetic) {
				if(genetic == undefined) return;
				this.product.genetic_id = genetic.id
			}
		},
		created(){
			this.getCategories();
		...
		methods:{
			...
			getGenetices() {
				axios.get('/get_genetices')
				.then(res => {
					this.genetices = res.data;
				})
			},
			getProducts(){
			...
			saveProduct(){
				...
				if(this.selectedGenetic == null) {
					alert('Select genetic');
					return;
				}
			...
			editProduct(product){
				...
				this.selectedCompany = this.companies.find(item => item.id == product.company_id);
				this.selectedGenetic = this.genetices.find(item => item.id == product.genetic_id);
